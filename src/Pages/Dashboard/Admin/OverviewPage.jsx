  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const resIssues = await axiosSecure.get(`/issues/user/${user.email}?limit=1000`);
        ...
        const resMonthly = await axiosSecure.get(`/issues/user/${user.email}/monthly`);
     
        setMonthlyData(resMonthly.data.monthly || []);

   
        const resPayments = await axiosSecure.get(`/payments/user/${user.email}`);
     
        setPayments(resPayments.data.total || 0);
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 rounded-lg shadow border border-gray-200">
          ...
          <p className="text-blue-600">Total Issues: {payload[0].value}</p>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyData} barSize={30}>
                ...
                <Legend />
                <Bar dataKey="total" name="Total Issues" radius={[8, 8, 0, 0]} fill="#3b82f6" />
