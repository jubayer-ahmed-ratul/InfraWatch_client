  ...
  return (
    <div className="p-6 bg-base-200 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-base-content">All Payments</h1>
      
      </div>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 text-center gap-4 mb-6">
        <div className="bg-base-100 shadow rounded p-4 border border-base-300">
          <p className="text-base-content/70">Total Payments</p>
          <p className="text-2xl font-bold text-base-content">{count}</p>
        </div>
        <div className="bg-base-100 shadow rounded p-4 border border-base-300">
          <p className="text-base-content/70">Total Amount</p>
          <p className="text-2xl font-bold text-base-content">{totalAmount} BDT</p>
        </div>
      </div>

   
      <div className="bg-base-100 shadow rounded-lg overflow-hidden border border-base-300">
          ...
          <table className="min-w-full divide-y divide-base-300">
            <thead className="bg-base-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-base-content/70 uppercase tracking-wider">Amount (BDT)</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-base-content/70 uppercase tracking-wider">Receipt</th>
              </tr>
            </thead>
            <tbody className="bg-base-100 divide-y divide-base-300">
              {filteredPayments.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-4 text-center text-base-content/60">No payments found</td>
                </tr>
              ) : (
                filteredPayments.map((payment, index) => (
                  <tr key={payment.id || payment._id || index} className="hover:bg-base-200">
                    <td className="px-6 py-4 whitespace-nowrap text-base-content">{index + 1}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-base-content">{payment.email}</td>
                    <td className="px-6 py-4 whitespace-nowrap font-semibold text-base-content">{payment.amount}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-base-content">{new Date(payment.date).toLocaleString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
